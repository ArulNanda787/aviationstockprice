...
# --- PAGE CONFIG ---
st.set_page_config(
    page_title="Southwest Airlines Forecast ✈️",
    page_icon="✈️",
    layout="wide",
...
/* HEADER */
.header {
    background: linear-gradient(135deg, #2E4BB1, #3453C4, #CCCCCC, #E51D23, #F9B612);
    color: #ffffff;
    text-align: center;
    padding: 45px 0;
    border-radius: 22px;
...
st.markdown("""
<div class='header'>
    <h1>✈️ Southwest Airlines Forecast</h1>
</div>
""", unsafe_allow_html=True)
...
if run_button:
    with st.spinner("Running time series decomposition and forecast... Please wait ⏳"):
        try:
            ...
            ts,loadings_df, explained_variance_ratio, fig_forecast, metrics, forecast_summary = Airline("southwest", steps)
